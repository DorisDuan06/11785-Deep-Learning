...
class BatchNorm(object):
    ...
    def backward(self, delta):
        ...
        dvar = np.sum(dnorm * (self.mean-self.x)*(self.var+self.eps)**(-3/2)/2,axis=0)
        dmean = np.sum(-dnorm / np.sqrt(self.var+self.eps),axis = 0)
        self.dbeta = np.sum(delta, axis = 0, keepdims=True)
        self.dgamma= np.sum(delta*self.norm, axis = 0,keepdims=True)
        return dnorm/np.sqrt(self.var+self.eps)+dvar*(2/len(delta)*(self.x-self.mean))+dmean/len(delta)
        # return out
