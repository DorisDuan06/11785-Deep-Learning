    ...
    def forward(self, x, h_0=None):
        ...
        for t in range(seq_len):
            ...
            for l in range(self.num_layers):
                ...
                h = self.rnn[l](h, self.hiddens[t][l])
                hidden = np.stack((hidden, h), axis=0) if hidden is not None else h
        ...
        logits = self.output_layer(self.hiddens[seq_len][-1])
        return logits
        # <--------------------------
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for t in range(seq_len-1, -1, -1):
            for l in range(self.num_layers-1, -1, -1):
                dh_prev_l, dh_prev_t = self.rnn[l].backward(dh[l], self.hiddens[t+1][l], self.hiddens[t+1][l-1] if l > 0 else self.x[:, t, :], self.hiddens[t][l])
                dh[l] = dh_prev_t
                if l > 0:
                    dh[l-1] += dh_prev_l
        dh_0 = dh / batch_size
